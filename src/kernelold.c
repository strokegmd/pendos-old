...
void prompt(void) {
    ...
    } else if (strstartswith(buffer, "setname ", 8)) {
        uint8_t* name = strrem(buffer, 8);
        fat_write_file(fat_find_file("NAME       "), name, strlen(name));
        tty_putchar('\n');
    } else if (strstartswith(buffer, "setpcname ", 10)) {
        uint8_t* pcname = strrem(buffer, 10);
        fat_write_file(fat_find_file("PC         "), pcname, strlen(pcname));
        tty_putchar('\n');
    } else if (strstartswith(buffer, "write ", 6)) {
        uint8_t* name = strrem(buffer, 6);
        uint8_t* content = strrem(buffer, 6+12);
        fat_write_file(fat_find_file(name), content, strlen(content));
        tty_putchar('\n');
    } else if (strstartswith(buffer, "touch ", 6)) {
        uint8_t* name = strrem(buffer, 6);
